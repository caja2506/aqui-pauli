// ==============================================
// COSTA RICA GEO — Datos geográficos para flujo de dirección
...
function getCantones(provincia) {
  const prov = _findKey(CR_GEO, provincia);
  return prov ? Object.keys(CR_GEO[prov]) : [];
}
...
function getDistritos(provincia, canton) {
  const prov = _findKey(CR_GEO, provincia);
  if (!prov) return [];
  const cant = _findKey(CR_GEO[prov], canton);
  return cant ? CR_GEO[prov][cant] : [];
}

/**
 * Buscar key case-insensitive con normalización de acentos
 */
function _findKey(obj, search) {
  if (!search) return null;
  const norm = _normalize(search);
  return Object.keys(obj).find(k => _normalize(k) === norm) || null;
}
